...
/// Macro to implement the [`OdeState`] trait for matrix types.
///
/// Matrix types implementing this trait must also implement the [`Matrix`] type.
///
/// # Example
///
/// This example demonstrates how we use the [`impl_ode_state_for_matrix`] macro internally within
/// the `numint` crate to implement the [`OdeState`] trait for [`Mat<f64>`].
///
/// ```ignore
/// use linalg_traits::{Mat, Matrix};
///
/// use numint::{impl_ode_state_for_matrix, OdeState};
///
/// impl_ode_state_for_matrix!(Mat<f64>);
/// ```
#[macro_export]
macro_rules! impl_ode_state_for_matrix {
    ($($type:ty),*) => {
        $(
...
#[cfg(feature = "nalgebra")]
impl_ode_state_for_matrix!(DMatrix<f64>);

// Implementation of OdeState for ndarray::Array2<f64>.
#[cfg(feature = "ndarray")]
impl_ode_state_for_matrix!(Array2<f64>);
