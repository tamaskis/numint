...
/// Macro to implement the [`OdeState`] trait for vector types.
///
/// Vector types implementing this trait must also implement the [`Vector`] type.
///
/// # Example
///
/// This example demonstrates how we use the [`impl_ode_state_for_vector`] macro internally within
/// the `numint` crate to implement the [`OdeState`] trait for [`Vec<f64>`].
///
/// ```ignore
/// use linalg_traits::Vector;
///
/// use numint::{impl_ode_state_for_vector, OdeState};
///
/// impl_ode_state_for_vector!(Vec<f64>);
/// ```
#[macro_export]
macro_rules! impl_ode_state_for_vector {
    ($($type:ty),*) => {
        $(
...
#[cfg(feature = "nalgebra")]
impl_ode_state_for_vector!(DVector<f64>);

// Implementation of OdeState for ndarray::Array1<f64>.
#[cfg(feature = "ndarray")]
impl_ode_state_for_vector!(Array1<f64>);
